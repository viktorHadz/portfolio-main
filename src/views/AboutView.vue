...
<template>
  ...
  <div class="relative flex h-40 w-40 items-center justify-center">
    <!-- Glow aura -->
    <div
      class="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_center,var(--acc-primary)_0%,transparent_70%)] opacity-50 blur-xl"
    ></div>

    <!-- Glass dome -->
    <div
      class="relative z-10 overflow-hidden rounded-full border border-[rgba(255,255,255,0.3)] bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.2)_0%,rgba(0,0,0,0.8)_90%)] shadow-[0_0_20px_var(--acc-primary)] backdrop-blur-md"
      style="aspect-ratio: 1/1"
    >
      <!-- Goo -->
      <div
        class="absolute bottom-0 left-0 h-1/2 w-full rounded-b-full bg-gradient-to-t from-[var(--acc-primary)] to-[rgba(0,255,200,0.1)] opacity-70"
      ></div>
      ...
      <img
        :src="icon"
        alt="project icon"
        class="absolute inset-0 m-auto h-1/2 w-1/2 object-contain select-none"
      />
    </div>

    <!-- Jar base -->
    <div
      class="absolute -bottom-2 h-4 w-2/3 rounded-full bg-[var(--bg-tertiary)] shadow-inner"
    ></div>
  </div>
</template>
<style scoped>
